...
<div   class="card card-primary ml-2 mt-3">
    <div class="card-header">
              ...
              <!-- /.card-header -->
              <!-- form start -->
              <form method="post" action="{{route('store.brands.update', $brand->id)}}">
                @csrf
                @method('PATCH')
                @foreach(LaravelLocalization::getSupportedLanguagesKeys() as $lang)
                <div class="form-group">
                    <label for="exampleInputEmail1">Title {{$lang}}</label>
                    <input name="title[{{$lang}}]" value="{{old('title.' .$lang, $brand->getTranslation('title', $lang))}}" type="text" class="form-control" id="exampleInputEmail1" value="{{$brand->getTranslation('title',$lang)}}" placeholder="Enter title">
                  </div>
                ...
                <div class="card-body">
                  ...
                  <select name="brand_id" class="custom-select form-control-border border-width-2" id="exampleSelectBorderWidth2">
                    ...
                    @foreach($brands as $relatedBrand)
                    <option value="{{$relatedBrand->id}}" {{$brand->brand_id == $relatedBrand->id ? 'selected' : '' }}>{{$relatedBrand->title}}</option>
                    @endforeach
